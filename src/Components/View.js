...
//                   <td>Deadline</td>
//                   <td>Pembuat user</td>
//                   <td>Pengguna mengakui 1</td>
//                   <td>Pengguna mengakui 2</td>
//                   <td>
//                     <ModalView/>
//                     <ModalEdit/>
//                     <ModalDelete/>
...
const View = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("/api/data"); // Endpoint untuk mengambil data dari server
      setData(response.data); // Menyimpan data dari respons ke state data
    } catch (error) {
      console.error("Error fetching data: ", error);
    }
                ...
                {data.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="gap-2">
                      <ModalView item={item} />
                      <ModalEdit item={item} />
                      <ModalDelete itemId={item.id} onDeleteSuccess={fetchData} />
                    </td>
                  </tr>
                ))}
              </tbody>
